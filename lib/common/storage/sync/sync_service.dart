...
import 'package:goncook/services/network/connection_observer.dart';
...
class SyncService {
  ...
  SyncService({required DatabaseHelper db, required TaskRepository taskRepository})
    : _db = db,
  ...
  void startSyncObserver() {
    ...
    ConnectionObserver.instance.onConnectionChanged.listen((isOnline) {
      _logger.info('Estado de conexión detectado: ${isOnline ? "ONLINE" : "OFFLINE"}');
  ...
  Future<void> syncPendingTasks() async {
    ...
    for (final task in pending) {
      _logger.info('Sincronizando tarea con ID: ${task.id}, operación: ${task.operation}');
      try {
        ...
        _logger.severe('Error al sincronizar tarea con ID ${task.id}', e, stackTrace);
      }
    }
    pending.isEmpty ? '' : _logger.info('Sincronización completada para todas las tareas.');
 */
  }
}
