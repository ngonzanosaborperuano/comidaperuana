...
void main() {
  ...
  group('login', () {
    test(
      'returns (true, "Login or registration successful") when type is LoginWith.withUserPassword',
      () async {
        final mockUserCredential = MockUserCredential();
        final mockUser = MockUser();
  ...
  group('loginWithEmail', () {
    test('returns true on successful login', () async {
      const user = AuthUser(email: 'test@example.com', contrasena: 'password');
      when(
        mockApiService.post(
          endpoint: anyNamed('endpoint'),
          body: anyNamed('body'),
        ...
        (_) async => const ApiResponse<AuthUser>(
          success: true,
          data: AuthUser(
            id: 1,
